...
public final class PublishSubject<T> extends Subject<T> {
    ...
    public final boolean a1() {
        if (((PublishDisposable[]) this.a.get()).length != 0) {
            return true;
        }
        return false;
    }
    ...
    /* JADX WARN: Multi-variable type inference failed */
    public final void d1(PublishDisposable publishDisposable) {
        PublishDisposable[] publishDisposableArr;
        while (true) {
            AtomicReference atomicReference = this.a;
            PublishDisposable[] publishDisposableArr2 = (PublishDisposable[]) atomicReference.get();
            if (publishDisposableArr2 != c && publishDisposableArr2 != (publishDisposableArr = t)) {
                int length = publishDisposableArr2.length;
                int i = 0;
                while (true) {
                    if (i < length) {
                        if (publishDisposableArr2[i] == publishDisposable) {
                            break;
                        } else {
                            i++;
                        }
