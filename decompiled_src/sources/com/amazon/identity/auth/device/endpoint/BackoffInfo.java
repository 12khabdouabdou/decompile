...
public class BackoffInfo {
    ...
    private long calculateBackoffInterval(long j) {
        ...
        if (j < j2) {
            ...
            StringBuilder E = AbstractC30172lva.E(j2, "Backoff interval cannot be less than ", " ms, set interval to ");
            E.append(j2);
            E.append(" ms");
            MAPLog.i(TAG, E.toString());
            return j2;
        }
    ...
    public BackoffInfo getNextBackoffInfo(URL url) {
        ...
        if (j - currentTimeMillis < MAX_BACKOFF_INTERVAL_WITH_JITTER_MS) {
            z2 = true;
        }
        if (z && z2) {
            return this;
        ...
        Locale locale = Locale.ENGLISH;
        MAPLog.i(TAG, "Last backoff interval is " + this.mBackoffInterval + " ms, updating backoff info...");
    ...
    public boolean shouldBackoff() {
        if (getBackoffIntervalFromNow() > 0) {
            return true;
        }
        return false;
    }
