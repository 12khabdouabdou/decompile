...
public class TokenVendor {
    ...
    private boolean isTokenAcceptable(AccessAtzToken accessAtzToken, Bundle bundle) {
        ...
        if (accessAtzToken != null && accessAtzToken.isRemainingLifeAcceptable(i)) {
            return true;
        }
        return false;
    }
                ...
                try {
                    if (strArr.length != 0) {
                        ...
                        if (authorizationTokens[1] != null) {
                            MAPLog.pii(str2, "Refresh token", "token=" + refreshAtzToken);
                            updateExistingRefreshToken(authorizationTokens[1], refreshAtzToken, context);
                            refreshAtzToken = (RefreshAtzToken) authorizationTokens[1];
                        }
                        ...
                        if (authorizationToken != null) {
                            ...
                            if (authorizationToken.insertOrUpdate(context)) {
                                if (z) {
                                    updateRequestedScopes(appInfo.getAppFamilyId(), strArr, context, (AccessAtzToken) authorizationToken, refreshAtzToken2, str);
                                }
