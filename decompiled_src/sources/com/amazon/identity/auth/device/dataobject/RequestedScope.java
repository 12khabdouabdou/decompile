...
public class RequestedScope extends AbstractDataObject implements Parcelable {
        ...
        if (obj instanceof RequestedScope) {
            try {
                ...
                if (this.scopeValue.equals(requestedScope.getScopeValue()) && this.appFamilyId.equals(requestedScope.getAppFamilyId()) && this.directedId.equals(requestedScope.getDirectedId()) && this.mAuthorizationAccessTokenId == requestedScope.getAuthorizationAccessTokenId()) {
                    if (this.mAuthorizationRefreshTokenId == requestedScope.getAuthorizationRefreshTokenId()) {
                        return true;
                    }
                }
                return false;
    ...
    public void setIsGranted(Boolean bool) {
        if (bool != isGranted()) {
            if (bool == null) {
                ...
                OUTCOME outcome2 = OUTCOME.REJECTED;
                setAuthorizationAccessTokenId(outcome2.longVal);
                setAuthorizationRefreshTokenId(outcome2.longVal);
            } else {
                OUTCOME outcome3 = OUTCOME.GRANTED_LOCALLY;
                setAuthorizationAccessTokenId(outcome3.longVal);
                setAuthorizationRefreshTokenId(outcome3.longVal);
            }
        }
    }
    ...
    public String toString() {
        ...
        return AbstractC30628mG8.p(sb, this.mAuthorizationRefreshTokenId, " }");
    }
