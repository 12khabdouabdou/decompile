...
public class PollingTask implements Runnable {
    ...
    public void run() {
        ...
        String str2 = LOG_TAG;
        MAPLog.i(str2, "Get the tokens from Code Pair successfully, update the database now");
        ...
        MAPLog.i(str2, "Insert new access token successfully");
        if (authorizationToken2.insert(this.mContext) == -1) {
            MAPLog.i(str2, "Failed to insert Refresh token");
            if (authorizationToken.delete(this.mContext)) {
                MAPLog.i(str2, "Deleted the access token!");
            } else {
        ...
        MAPLog.i(str2, "Insert new refresh token successfully");
        if (new RequestedScope(this.mScopeString, this.mAppInfo.getAppFamilyId(), null, authorizationToken.getRowId(), authorizationToken2.getRowId()).insert(this.mContext) == -1) {
            ...
            MAPLog.i(str2, "Insert new requestedScope successfully");
