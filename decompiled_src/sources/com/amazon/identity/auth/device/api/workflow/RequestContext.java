...
public final class RequestContext {
    ...
    public <T> Set<T> getListenersInternal(String str, Class<T> cls) throws ListenerNotFoundException {
        ...
        if (str != null) {
            ...
            StringBuilder s = AbstractC31823n9f.s("No listeners were registered with type \"", str, "\" for RequestContext ");
            s.append(this.requestContextId);
            s.append(". Listener types present: ");
            s.append(this.listeners.keySet());
            throw new ListenerNotFoundException(s.toString());
        }
                ...
                ThreadUtils.THREAD_POOL.execute(new Runnable() { // from class: com.amazon.identity.auth.device.api.workflow.RequestContext.1
                    ...
                    public void run() {
                        try {
                            ...
                            MAPLog.e(RequestContext.LOG_TAG, "RequestContext " + RequestContext.this.requestContextId + ": Unable to handle activity result", e);
                        }
                    }
                });
                return;
