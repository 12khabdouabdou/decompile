...
public final class AuthorizationManager {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (scope.getScopeData() != null) {
                try {
                    ...
                    MAPLog.pii(LOG_TAG, EU0.B("Unable to serialize scope data for scope \"", name, "\""), scope.getScopeData().toString(), e);
                }
            }
        }
    ...
    private static String buildXAmazonOptions(AuthorizeRequest authorizeRequest) {
        ...
        for (Workflow workflow : authorizeRequest.getWorkflows()) {
            ...
            try {
                ...
                MAPLog.pii(LOG_TAG, EU0.B("Unable to serialize workflow data for workflow \"", name, "\""), workflowData.toString(), e);
            }
        }
        try {
    ...
    public static void setSandboxMode(Context context, boolean z) {
        ...
        MAPLog.i(LOG_TAG, "Changing sandbox mode from " + isSandboxMode + " to " + z);
        if (isSandboxMode != z) {
