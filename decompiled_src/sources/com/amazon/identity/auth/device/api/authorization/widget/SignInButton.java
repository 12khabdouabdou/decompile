...
public class SignInButton extends ImageButton {
    ...
    private String getButtonDescription() {
        ...
        return AbstractC30172lva.A(" }", DM4.v("Button configuration = { style:", obj, " color:", obj2, " pressed:"), isPressed());
    }
    ...
    private int getResourceIdForCurrentState() {
        ...
        if (num == null) {
            int identifier = getResources().getIdentifier(AbstractC30172lva.y(getContext().getPackageName(), ":drawable/", buttonNameForCurrentState), null, null);
            Integer valueOf = Integer.valueOf(identifier);
            if (identifier != 0) {
                map.put(buttonNameForCurrentState, valueOf);
            } else {
                String str = LOG_TAG;
                StringBuilder s = AbstractC31823n9f.s("Could not find the resource ID for the image named \"", buttonNameForCurrentState, "\". It must be added to the drawables resources  (");
                s.append(getButtonDescription());
                s.append(")");
                MAPLog.e(str, s.toString());
            }
            num = valueOf;
        }
        return num.intValue();
