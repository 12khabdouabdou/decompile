...
public class ThirdPartyAuthorizationHelper extends AuthorizationHelper {
    ...
    public void authorize(final AuthorizeRequest authorizeRequest, final Context context, String str, final String str2, String str3, String[] strArr, final boolean z, TokenVendor tokenVendor, final AuthorizationListener authorizationListener, Bundle bundle) throws AuthError {
        ...
        if (!ThreadUtils.isRunningOnMainThread()) {
            ...
            bundle2.putBoolean(AuthzConstants.BUNDLE_KEY.CHECK_API_KEY.val, false);
            bundle2.putBoolean(AuthzConstants.BUNDLE_KEY.RETURN_CODE.val, true);
            bundle2.putString(LWAConstants.AUTHORIZE_BUNDLE_KEY.REGION.val, AuthorizationManager.getRegion(context).getStringValue());
            bundle2.putString(AuthzConstants.BUNDLE_KEY.CLIENT_ID.val, str2);
            bundle2.putString(AuthzConstants.BUNDLE_KEY.SDK_VERSION.val, "LWAAndroidSDK3.0.7");
            try {
                bundle2.putBundle(AuthzConstants.BUNDLE_KEY.EXTRA_URL_PARAMS.val, getExtraUrlParams(bundle2));
                if (!z2 && (StoredPreferences.isTokenObtainedFromSSO(context) || cachedScopes == null || cachedScopes.size() == 0)) {
                    bundle3 = startAuthorizationWithService(context, commonScopesForAuthorization, bundle2);
                }
                if (bundle3.containsKey("code") && !TextUtils.isEmpty(bundle3.getString("code"))) {
                    if (bundle2.getBoolean(AuthzConstants.BUNDLE_KEY.GET_AUTH_CODE.val, false)) {
                        AuthorizationHelper.sendAuthorizationCodeAsResponse(bundle3.getString("code"), str2, str3, authorizationListener);
                        return;
                    }
                    handleCodeForTokenExchange(context, str, this.codeChallengeWorkflow.getCodeVerifier(), bundle3, bundle2, authorizationListener);
                    StoredPreferences.setTokenObtainedFromSSO(context, true);
                    return;
                }
                Bundle bundle4 = bundle3;
                final Bundle bundle5 = bundle2;
                if ((!bundle4.containsKey(AuthError.AUTH_ERROR_EXECEPTION) && !bundle4.containsKey(AuthzConstants.BUNDLE_KEY.AUTHORIZE.val) && !bundle4.containsKey(AuthzConstants.BUNDLE_KEY.CAUSE_ID.val)) || bundle4.getBoolean(START_AUTHORIZATION_AUTH_ERROR)) {
                    ...
                    new Handler(Looper.getMainLooper()).post(new Runnable() { // from class: com.amazon.identity.auth.device.authorization.ThirdPartyAuthorizationHelper.1
                        ...
                        public void run() {
                            try {
                                ...
                                thirdPartyAuthorizationHelper.authorizeWithBrowser(authorizeRequest2, context2, context2.getPackageName(), str2, commonScopesForAuthorization, authorizationListener, bundle5, appInfo);
