...
public class InternalAuthManager {
        ...
        if (strArr != null && strArr.length != 0) {
            ...
            ThreadUtils.THREAD_POOL.execute(new Runnable() { // from class: com.amazon.identity.auth.device.authorization.InternalAuthManager.1
                ...
                public void run() {
                    ...
                    if (!bundle3.containsKey(bundle_key.val)) {
                        bundle3.putBoolean(bundle_key.val, AuthorizationManager.isSandboxMode(context));
                    }
                    ...
                    try {
                        ...
                        thirdPartyAuthorizationHelper.authorize(authorizeRequest2, context2, context2.getPackageName(), InternalAuthManager.this.clientId, InternalAuthManager.this.getRedirectURI(context), strArr, true, InternalAuthManager.tokenVendor, authorizationListener, bundle3);
                    } catch (AuthError e) {
            ...
            public void run() {
                if (InternalAuthManager.this.isAPIKeyValid(context)) {
                    ...
                    if (clearServerSideAuthorizationState == null && clearSSOSideAuthorizationState == null) {
                        ...
                        if (clearSSOSideAuthorizationState != null) {
    ...
    public Region getRegion(Context context) {
        ...
        if (Region.AUTO == region) {
            return LWAEnvironment.getEndpointDomainBuilder(context, this.appInfo).getRegionForAPIKey();
        }
        return region;
    }
    ...
    public boolean isAPIKeyValid(Context context) {
        if (!appIdentifier.isAPIKeyValid(context) || this.clientId == null) {
            return false;
        }
        return true;
    }
