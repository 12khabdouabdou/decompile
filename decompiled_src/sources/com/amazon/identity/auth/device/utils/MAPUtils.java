...
public final class MAPUtils {
    ...
    public static void deleteDatabase(Context context) {
        try {
            ...
            MAPLog.d(str, "deleteDatabase was successful : " + context.deleteDatabase(DatabaseHelper.MAP_DB_NAME));
        ...
        try {
            ...
            if (!TextUtils.isEmpty(protocol) && protocol.contains(PROTOCOL)) {
                ...
                if (!TextUtils.isEmpty(host) && host.contains(AMAZON_HOST)) {
                    ...
                    String str2 = LOG_TAG;
                    StringBuilder t = AbstractC30628mG8.t(" isEmpty=", "startsWithAP=", "equalsGP=", isEmpty, startsWith);
                    t.append(equals);
                    t.append("equalsForgotPassword=");
                    t.append(equals2);
                    MAPLog.d(str2, t.toString());
        ...
        synchronized (MAPUtils.class) {
            try {
                if (SHOULD_USE_DE == null) {
                    if (PlatformUtils.isPlatformFireOS(context) && Build.VERSION.SDK_INT >= 30) {
                        z = true;
                    } else {
                        z = false;
                    }
                    SHOULD_USE_DE = Boolean.valueOf(z);
                }
    ...
    public static String toDelimitedString(String[] strArr, String str) {
        ...
        if (strArr != null && strArr.length > 0) {
            ...
            for (int i = 0; i < strArr.length; i++) {
                StringBuilder F = AbstractC30172lva.F(str3);
                F.append(strArr[i].trim());
            ...
            if (appInfo != null && appInfo.getAllowedScopes() != null) {
                ...
                for (String str2 : strArr) {
                    if (str2 != null && hashSet.contains(str2)) {
                        if (scope_modifier == SCOPE_MODIFIER.REMOTE && Scope.isLocal(str2)) {
