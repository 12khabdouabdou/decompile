...
import defpackage.AbstractC25563iU;
...
public abstract class SCameraEffectProcessor extends SProcessor {
    ...
    public class SemEventCallback extends AbstractSemCameraEffectProcessor.EventCallback {
        ...
        public void onCameraConfigUpdated(AbstractCameraConfigParameter abstractCameraConfigParameter, final boolean z) {
            ...
            if (abstractCameraConfigParameter == AbstractSemCameraEffectProcessor.CAMERA_CONFIG_VDIS_CONTROL_MODE) {
                cameraConfigParameter = SCameraEffectProcessor.CAMERA_CONFIG_VDIS_CONTROL_MODE;
            } else if (abstractCameraConfigParameter == AbstractSemCameraEffectProcessor.CAMERA_CONFIG_LIVE_HDR_OPERATION_MODE) {
                cameraConfigParameter = SCameraEffectProcessor.CAMERA_CONFIG_LIVE_HDR_OPERATION_MODE;
            } else {
                cameraConfigParameter = null;
            }
            SCameraEffectProcessor sCameraEffectProcessor = SCameraEffectProcessor.this;
    ...
    @RequiresApi(api = 27)
    public void handleHumanSegmentationCallback(Bundle bundle) {
        SHumanSegmentationEngine sHumanSegmentationEngine;
        int size;
        ByteBuffer mapReadWrite;
        int size2;
        bundle.setClassLoader(AbstractC25563iU.f().getClassLoader());
        SharedMemory d = AbstractC25563iU.d(bundle.getParcelable("data"));
        try {
            try {
                sHumanSegmentationEngine = (SHumanSegmentationEngine) this.mActiveEngine.get(1);
            ...
            if (sHumanSegmentationEngine == null) {
                if (d != null) {
                    d.close();
                    return;
                }
            ...
            SHumanSegmentationEngine.HumanSegCallback callback = sHumanSegmentationEngine.getCallback();
            if (callback == null) {
                if (d != null) {
                    d.close();
                    return;
            ...
            if (d != null) {
                ...
                try {
                    ...
                    mapReadWrite = d.mapReadWrite();
                    size2 = d.getSize();
                    mapReadWrite.get(bArr, 0, size2);
                    callback.onHumanSegmentData(bArr);
                } catch (OutOfMemoryError unused) {
                    callback.onHumanSegmentData(null);
                }
            } else {
                callback.onHumanSegmentData(null);
            }
                    ...
                    if (t != null) {
                        if (cameraConfigParameter == CAMERA_CONFIG_SDK_OPERATION_MODE) {
                            ...
                            if (t == CameraConfigParameter.OperationMode.FULL) {
                                this.mOpMode = AbstractCameraConfigParameter.OperationMode.FULL;
                            } else if (t == CameraConfigParameter.OperationMode.HDR_ONLY) {
                                this.mOpMode = AbstractCameraConfigParameter.OperationMode.HDR_ONLY;
                            } else if (t == CameraConfigParameter.OperationMode.LOW_LIGHT_ONLY) {
                                this.mOpMode = AbstractCameraConfigParameter.OperationMode.LOW_LIGHT_ONLY;
                            }
                            ...
                            if (t == CameraConfigParameter.SuperNightOperationMode.SUPER_NIGHT_AUTO) {
                                this.mNightOpMode = AbstractCameraConfigParameter.SuperNightOperationMode.SUPER_NIGHT_AUTO;
                            } else if (t == CameraConfigParameter.SuperNightOperationMode.SUPER_NIGHT_OFF) {
                                this.mNightOpMode = AbstractCameraConfigParameter.SuperNightOperationMode.SUPER_NIGHT_OFF;
                            }
                        } else if (cameraConfigParameter == CAMERA_CONFIG_VDIS_CONTROL_MODE) {
                            if (t == CameraConfigParameter.VideoVDISMode.VIDEO_VDIS_MODE_ON) {
                                this.mVDISMode = AbstractCameraConfigParameter.VideoVDISMode.VIDEO_VDIS_MODE_ON;
                            } else if (t == CameraConfigParameter.VideoVDISMode.VIDEO_VDIS_MODE_OFF) {
                                this.mVDISMode = AbstractCameraConfigParameter.VideoVDISMode.VIDEO_VDIS_MODE_OFF;
                            }
                            ...
                            if (t == CameraConfigParameter.VideoLiveHDRMode.VIDEO_HDR_OFF) {
                                this.mLiveHdrMode = AbstractCameraConfigParameter.VideoLiveHDRMode.VIDEO_HDR_OFF;
                            } else if (t == CameraConfigParameter.VideoLiveHDRMode.VIDEO_HDR_ON) {
                                this.mLiveHdrMode = AbstractCameraConfigParameter.VideoLiveHDRMode.VIDEO_HDR_ON;
                            }
                        } else {
