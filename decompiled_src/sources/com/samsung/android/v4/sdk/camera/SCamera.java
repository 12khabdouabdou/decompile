...
public class SCamera {
    ...
    private boolean isSDKServiceOnDevice(Context context) {
        try {
            if (context.getPackageManager().getPackageInfo(SCAMERA_SERVICE_PACKAGE, 0) == null) {
                return false;
            }
            return true;
    ...
    public int checkAvailability(Context context) {
        if (context != null) {
            ...
            if (isSDKServiceOnDevice(context) && isSEPSupported(context) && !isNonCompatibleBinary()) {
                return 0;
            }
            return 2;
        }
    ...
    public String[] getCameraIdList(Context context) throws CameraAccessException {
        if (context != null) {
            if (checkAvailability(context) == 0) {
                return getSemCamera(context).getCameraIdList(context);
            }
