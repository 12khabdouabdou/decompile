...
public abstract class AbstractSemCamera {
    ...
    private static final boolean isSemAvailable(Context context) {
        if (context != null) {
            if (context.getPackageManager().hasSystemFeature(SEM_FEATURE_SAMSUNG_EXPERIENCE_MOBILE) || context.getPackageManager().hasSystemFeature(SEM_FEATURE_SAMSUNG_LITE_EXPERIENCE_MOBILE)) {
                return true;
            }
            return false;
        }
