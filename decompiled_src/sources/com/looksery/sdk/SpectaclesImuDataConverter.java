    ...
    public ImuData[] convertImuData(ImuData[] imuDataArr) {
        for (ImuData imuData : imuDataArr) {
            if (imuData.acceleration.length != 3 || imuData.rotationRate.length != 3) {
                throw new IllegalArgumentException(AbstractC31823n9f.r("Acceleration length: ", " != 3 or Rotation length: ", " != 3", imuData.acceleration.length, imuData.rotationRate.length));
            }
        }
