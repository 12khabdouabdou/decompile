...
public final class CodecFactory {
    ...
    private boolean isMotoG6() {
        ...
        if (!str.equals("moto g(6)") && !str.equals("moto g(6) plus")) {
            return false;
        }
        return true;
    }
        ...
        try {
            try {
                ...
                if (Build.VERSION.SDK_INT >= 23) {
                    i = mediaCodec.getCodecInfo().getCapabilitiesForType("video/avc").getMaxSupportedInstances();
                } else {
                    i = 16;
                }
    ...
    public final MediaCodecWrapper videoDecoder() throws IOException {
        ...
        int i;
        if (!this.codecSettings.useSoftwareDecoder() && !isMotoG6() && !isSamsung10Th()) {
            createByCodecName = MediaCodec.createDecoderByType("video/avc");
        } else {
            createByCodecName = MediaCodec.createByCodecName(DECODER_SOFTWARE);
        }
