...
public final class DogTagObserver<T> implements Observer<T>, LambdaConsumerIntrospection {
    ...
    public boolean hasCustomOnError() {
        ...
        if ((observer instanceof LambdaConsumerIntrospection) && ((LambdaConsumerIntrospection) observer).hasCustomOnError()) {
            return true;
        }
        return false;
    }
    ...
    public void onComplete() {
        if (this.config.guardObserverCallbacks) {
            RxDogTag.guardedDelegateCall(new g(this, 2), new LQ1(3, this.delegate));
        } else {
