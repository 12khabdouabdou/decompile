package defpackage;

import java.util.ArrayList;
import java.util.Set;

/* renamed from: bfb, reason: case insensitive filesystem */
/* loaded from: classes8.dex */
public class C16445bfb extends AbstractC24724hqj {
    public String A;
    public Long B;
    public String C;
    public Long D;
    public EnumC20324eZ2 E;
    public Boolean F;
    public ArrayList G;
    public ArrayList H;
    public String j;
    public String k;
    public String l;
    public String m;
    public String n;
    public String o;
    public EnumC30902mTb p;
    public SPg q;
    public EnumC2472Elb r;
    public String s;
    public Long t;
    public String u;
    public Long v;
    public Long w;
    public Long x;
    public EnumC1388Clb y;
    public EnumC1930Dlb z;

    public C16445bfb() {
        super("MDP_MEDIA_ORCHESTRATION", EnumC1516Cre.BUSINESS, 1.0d, 1.0d, 1.0d);
    }

    @Override // defpackage.InterfaceC5193Jje
    public final void c(AK3 ak3, Set set) {
        byte[] bArr = new byte[4];
        AbstractC20835ew8.y0(ak3, 2, bArr, this.z, set);
        AbstractC20835ew8.y0(ak3, 3, bArr, this.q, set);
        AbstractC20835ew8.B0(ak3, 4, bArr, this.n, set);
        AbstractC20835ew8.z0(ak3, 5, bArr, this.D, set);
        AbstractC20835ew8.B0(ak3, 6, bArr, this.k, set);
        AbstractC20835ew8.B0(ak3, 7, bArr, this.o, set);
        AbstractC20835ew8.y0(ak3, 8, bArr, this.y, set);
        AbstractC20835ew8.B0(ak3, 9, bArr, this.u, set);
        AbstractC20835ew8.z0(ak3, 10, bArr, this.t, set);
        AbstractC20835ew8.B0(ak3, 11, bArr, this.s, set);
        AbstractC20835ew8.B0(ak3, 12, bArr, this.C, set);
        AbstractC20835ew8.z0(ak3, 13, bArr, this.B, set);
        AbstractC20835ew8.y0(ak3, 14, bArr, this.r, set);
        AbstractC20835ew8.z0(ak3, 15, bArr, this.v, set);
        AbstractC20835ew8.K0(ak3, 16, bArr, this.H, set);
        AbstractC20835ew8.K0(ak3, 17, bArr, this.G, set);
        AbstractC20835ew8.B0(ak3, 18, bArr, this.l, set);
        AbstractC20835ew8.B0(ak3, 19, bArr, this.m, set);
        AbstractC20835ew8.B0(ak3, 20, bArr, this.j, set);
        AbstractC20835ew8.z0(ak3, 21, bArr, this.x, set);
        AbstractC20835ew8.z0(ak3, 22, bArr, this.w, set);
        AbstractC20835ew8.B0(ak3, 24, bArr, this.A, set);
        AbstractC20835ew8.y0(ak3, 25, bArr, this.p, set);
        AbstractC20835ew8.y0(ak3, 26, bArr, this.E, set);
        AbstractC20835ew8.w0(ak3, 27, bArr, this.F, set);
        ak3.m(bArr);
    }

    @Override // defpackage.InterfaceC5193Jje
    public final int d() {
        return 1400;
    }
}
