package defpackage;

import java.util.Map;
import java.util.Set;

/* renamed from: wBd, reason: case insensitive filesystem */
/* loaded from: classes8.dex */
public class C43902wBd extends TBd {
    public EnumC39892tBd r;
    public String s;
    public String t;
    public Long u;

    public C43902wBd() {
        super("PLUS_MANAGEMENT_PAGE_ITEM_IMPRESSION", EnumC1516Cre.BUSINESS, 1.0d, 1.0d, 1.0d);
    }

    @Override // defpackage.InterfaceC5193Jje
    public final void c(AK3 ak3, Set set) {
        byte[] bArr = new byte[2];
        AbstractC20835ew8.B0(ak3, 2, bArr, this.t, set);
        AbstractC20835ew8.y0(ak3, 3, bArr, this.r, set);
        AbstractC20835ew8.B0(ak3, 4, bArr, this.k, set);
        AbstractC20835ew8.y0(ak3, 5, bArr, this.j, set);
        AbstractC20835ew8.B0(ak3, 6, bArr, this.o, set);
        AbstractC20835ew8.B0(ak3, 7, bArr, this.m, set);
        AbstractC20835ew8.y0(ak3, 8, bArr, this.l, set);
        AbstractC20835ew8.y0(ak3, 9, bArr, this.n, set);
        AbstractC20835ew8.B0(ak3, 10, bArr, this.q, set);
        AbstractC20835ew8.y0(ak3, 11, bArr, this.p, set);
        AbstractC20835ew8.z0(ak3, 12, bArr, this.u, set);
        AbstractC20835ew8.B0(ak3, 13, bArr, this.s, set);
        ak3.m(bArr);
    }

    @Override // defpackage.InterfaceC5193Jje
    public final int d() {
        return 4070;
    }

    @Override // defpackage.TBd, defpackage.AbstractC24724hqj, defpackage.MR6, defpackage.AXa
    public final int e(Map map) {
        int e = super.e(map);
        String str = (String) map.get("item_id");
        this.t = str;
        if (str != null) {
            e++;
        }
        Long l = (Long) map.get("item_index");
        this.u = l;
        if (l != null) {
            e++;
        }
        if (map.containsKey("item_type")) {
            Object obj = map.get("item_type");
            if (obj instanceof String) {
                this.r = EnumC39892tBd.valueOf((String) obj);
            } else {
                this.r = (EnumC39892tBd) obj;
            }
            e++;
        }
        String str2 = (String) map.get("item_type_specific");
        this.s = str2;
        if (str2 != null) {
            return e + 1;
        }
        return e;
    }
}
