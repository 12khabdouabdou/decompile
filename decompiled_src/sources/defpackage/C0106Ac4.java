package defpackage;

import java.util.Set;

/* renamed from: Ac4, reason: case insensitive filesystem */
/* loaded from: classes8.dex */
public class C0106Ac4 extends AbstractC20391ec4 {
    public Long A;
    public String B;
    public Boolean y;
    public String z;

    public C0106Ac4() {
        super("CREATIVE_KIT_ERROR_EVENT", EnumC1516Cre.BUSINESS, 1.0d, 0.1d, 1.0d);
    }

    @Override // defpackage.InterfaceC5193Jje
    public final void c(AK3 ak3, Set set) {
        byte[] bArr = new byte[3];
        AbstractC20835ew8.B0(ak3, 2, bArr, this.B, set);
        AbstractC20835ew8.y0(ak3, 3, bArr, this.p, set);
        AbstractC20835ew8.y0(ak3, 4, bArr, this.q, set);
        AbstractC20835ew8.B0(ak3, 6, bArr, this.x, set);
        AbstractC20835ew8.B0(ak3, 7, bArr, this.z, set);
        AbstractC20835ew8.w0(ak3, 8, bArr, this.v, set);
        AbstractC20835ew8.w0(ak3, 9, bArr, this.u, set);
        AbstractC20835ew8.w0(ak3, 10, bArr, this.s, set);
        AbstractC20835ew8.w0(ak3, 11, bArr, this.t, set);
        AbstractC20835ew8.w0(ak3, 12, bArr, this.r, set);
        AbstractC20835ew8.z0(ak3, 13, bArr, this.A, set);
        AbstractC20835ew8.w0(ak3, 14, bArr, this.y, set);
        AbstractC20835ew8.w0(ak3, 15, bArr, this.o, set);
        AbstractC20835ew8.w0(ak3, 17, bArr, this.w, set);
        AbstractC20835ew8.B0(ak3, 18, bArr, this.m, set);
        AbstractC20835ew8.y0(ak3, 19, bArr, this.n, set);
        AbstractC20835ew8.y0(ak3, 21, bArr, this.k, set);
        AbstractC20835ew8.B0(ak3, 22, bArr, this.l, set);
        AbstractC20835ew8.B0(ak3, 23, bArr, this.j, set);
        ak3.m(bArr);
    }

    @Override // defpackage.InterfaceC5193Jje
    public final int d() {
        return 4149;
    }
}
