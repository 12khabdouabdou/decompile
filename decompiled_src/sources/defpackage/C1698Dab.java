package defpackage;

import java.util.Set;

/* renamed from: Dab, reason: case insensitive filesystem */
/* loaded from: classes8.dex */
public class C1698Dab extends AbstractC24724hqj {
    public Long j;
    public Long k;
    public R7b l;
    public EnumC29743lc m;
    public Long n;
    public Long o;
    public Long p;
    public Long q;
    public Double r;
    public Long s;
    public Boolean t;
    public String u;
    public Double v;
    public Long w;
    public String x;
    public String y;
    public Long z;

    public C1698Dab() {
        super("MAP_USER_VIEW", EnumC1516Cre.BUSINESS, 1.0d, 1.0d, 1.0d);
    }

    @Override // defpackage.InterfaceC5193Jje
    public final void c(AK3 ak3, Set set) {
        byte[] bArr = new byte[3];
        AbstractC20835ew8.y0(ak3, 2, bArr, this.m, set);
        AbstractC20835ew8.x0(ak3, 3, bArr, this.r, set);
        AbstractC20835ew8.w0(ak3, 4, bArr, this.t, set);
        AbstractC20835ew8.z0(ak3, 5, bArr, this.p, set);
        AbstractC20835ew8.z0(ak3, 6, bArr, this.o, set);
        AbstractC20835ew8.z0(ak3, 7, bArr, this.j, set);
        AbstractC20835ew8.y0(ak3, 8, bArr, this.l, set);
        AbstractC20835ew8.z0(ak3, 9, bArr, this.n, set);
        AbstractC20835ew8.B0(ak3, 11, bArr, this.u, set);
        AbstractC20835ew8.z0(ak3, 13, bArr, this.s, set);
        AbstractC20835ew8.z0(ak3, 15, bArr, this.q, set);
        AbstractC20835ew8.x0(ak3, 16, bArr, this.v, set);
        AbstractC20835ew8.B0(ak3, 18, bArr, this.x, set);
        AbstractC20835ew8.z0(ak3, 20, bArr, this.w, set);
        AbstractC20835ew8.B0(ak3, 23, bArr, this.y, set);
        AbstractC20835ew8.z0(ak3, 24, bArr, this.k, set);
        AbstractC20835ew8.z0(ak3, 25, bArr, this.z, set);
        ak3.m(bArr);
    }

    @Override // defpackage.InterfaceC5193Jje
    public final int d() {
        return 1398;
    }
}
