package defpackage;

import com.google.protobuf.nano.MessageNano;

/* renamed from: rX6, reason: case insensitive filesystem */
/* loaded from: classes7.dex */
public final class C37669rX6 extends AbstractC32978o17 {
    public int a = 0;
    public C35622q[] b = C35622q.a();
    public int c = 0;

    public C37669rX6() {
        this.unknownFieldData = null;
        this.cachedSize = -1;
    }

    @Override // defpackage.AbstractC32978o17, com.google.protobuf.nano.MessageNano
    public final int computeSerializedSize() {
        int computeSerializedSize = super.computeSerializedSize();
        C35622q[] c35622qArr = this.b;
        if (c35622qArr != null && c35622qArr.length > 0) {
            int i = 0;
            while (true) {
                C35622q[] c35622qArr2 = this.b;
                if (i >= c35622qArr2.length) {
                    break;
                }
                C35622q c35622q = c35622qArr2[i];
                if (c35622q != null) {
                    computeSerializedSize = C39067sa3.l(1, c35622q) + computeSerializedSize;
                }
                i++;
            }
        }
        if ((this.a & 1) != 0) {
            return C39067sa3.i(2, this.c) + computeSerializedSize;
        }
        return computeSerializedSize;
    }

    @Override // com.google.protobuf.nano.MessageNano
    public final MessageNano mergeFrom(C36392qa3 c36392qa3) {
        int length;
        while (true) {
            int u = c36392qa3.u();
            if (u == 0) {
                break;
            }
            if (u != 10) {
                if (u != 16) {
                    if (!storeUnknownField(c36392qa3, u)) {
                        break;
                    }
                } else {
                    int q = c36392qa3.q();
                    if (q == 0 || q == 1 || q == 2 || q == 3 || q == 4) {
                        this.c = q;
                        this.a |= 1;
                    }
                }
            } else {
                int E = AbstractC19498dw8.E(c36392qa3, 10);
                C35622q[] c35622qArr = this.b;
                if (c35622qArr == null) {
                    length = 0;
                } else {
                    length = c35622qArr.length;
                }
                int i = E + length;
                C35622q[] c35622qArr2 = new C35622q[i];
                if (length != 0) {
                    System.arraycopy(c35622qArr, 0, c35622qArr2, 0, length);
                }
                while (length < i - 1) {
                    C35622q c35622q = new C35622q();
                    c35622qArr2[length] = c35622q;
                    c36392qa3.k(c35622q);
                    c36392qa3.u();
                    length++;
                }
                C35622q c35622q2 = new C35622q();
                c35622qArr2[length] = c35622q2;
                c36392qa3.k(c35622q2);
                this.b = c35622qArr2;
            }
        }
        return this;
    }

    @Override // defpackage.AbstractC32978o17, com.google.protobuf.nano.MessageNano
    public final void writeTo(C39067sa3 c39067sa3) {
        C35622q[] c35622qArr = this.b;
        if (c35622qArr != null && c35622qArr.length > 0) {
            int i = 0;
            while (true) {
                C35622q[] c35622qArr2 = this.b;
                if (i >= c35622qArr2.length) {
                    break;
                }
                C35622q c35622q = c35622qArr2[i];
                if (c35622q != null) {
                    c39067sa3.K(1, c35622q);
                }
                i++;
            }
        }
        if ((this.a & 1) != 0) {
            c39067sa3.I(2, this.c);
        }
        super.writeTo(c39067sa3);
    }
}
