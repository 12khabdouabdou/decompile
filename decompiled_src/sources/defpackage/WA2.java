package defpackage;

import com.amazon.identity.auth.map.device.AccountManagerConstants;
import com.google.ar.core.ImageMetadata;
import com.google.protobuf.nano.MessageNano;
import com.snapchat.client.chrysalis.Chrysalis;
import com.snapchat.client.messaging.Tweaks;
import io.requery.android.database.sqlite.SQLiteDatabase;
import org.opencv.imgproc.Imgproc;

/* loaded from: classes9.dex */
public final class WA2 extends AbstractC32978o17 {
    public int a = 0;
    public boolean b = false;
    public boolean c = false;
    public int t = 0;
    public int X = 0;
    public int Y = 0;
    public int Z = 0;
    public int e0 = 0;
    public int f0 = 0;
    public int g0 = 0;
    public int h0 = 0;
    public int i0 = 0;
    public int j0 = 0;
    public int k0 = 0;
    public int l0 = 0;
    public int m0 = 0;
    public int n0 = 0;
    public int o0 = 0;
    public int p0 = 0;
    public int q0 = 0;
    public boolean r0 = false;
    public int s0 = 0;
    public int t0 = 0;
    public int u0 = 0;
    public int v0 = 0;
    public int w0 = 0;
    public int x0 = 0;
    public int y0 = 0;
    public int z0 = 0;
    public boolean A0 = false;
    public boolean B0 = false;
    public int C0 = 0;
    public int D0 = 0;
    public int E0 = 0;
    public int F0 = 0;

    public WA2() {
        this.unknownFieldData = null;
        this.cachedSize = -1;
    }

    @Override // defpackage.AbstractC32978o17, com.google.protobuf.nano.MessageNano
    public final int computeSerializedSize() {
        int a = C39067sa3.a(2) + C39067sa3.a(1) + super.computeSerializedSize();
        if ((1 & this.a) != 0) {
            a += C39067sa3.i(3, this.t);
        }
        if ((2 & this.a) != 0) {
            a += C39067sa3.i(4, this.X);
        }
        if ((this.a & 4) != 0) {
            a += C39067sa3.i(5, this.Y);
        }
        if ((this.a & 8) != 0) {
            a += C39067sa3.i(6, this.Z);
        }
        if ((this.a & 16) != 0) {
            a += C39067sa3.i(7, this.e0);
        }
        if ((this.a & 32) != 0) {
            a += C39067sa3.i(8, this.f0);
        }
        if ((this.a & 64) != 0) {
            a += C39067sa3.i(9, this.g0);
        }
        if ((this.a & 128) != 0) {
            a += C39067sa3.s(10, this.h0);
        }
        if ((this.a & 256) != 0) {
            a += C39067sa3.s(11, this.i0);
        }
        if ((this.a & Chrysalis.PIXEL_LAYOUT_ARGB) != 0) {
            a += C39067sa3.s(12, this.j0);
        }
        if ((this.a & 1024) != 0) {
            a += C39067sa3.s(13, this.k0);
        }
        if ((this.a & 2048) != 0) {
            a += C39067sa3.s(14, this.l0);
        }
        if ((this.a & 4096) != 0) {
            a += C39067sa3.s(15, this.m0);
        }
        if ((this.a & 8192) != 0) {
            a += C39067sa3.s(16, this.n0);
        }
        if ((this.a & 16384) != 0) {
            a += C39067sa3.s(17, this.o0);
        }
        if ((this.a & SQLiteDatabase.OPEN_NOMUTEX) != 0) {
            a += C39067sa3.s(18, this.p0);
        }
        if ((this.a & 65536) != 0) {
            a += C39067sa3.s(19, this.q0);
        }
        if ((this.a & 131072) != 0) {
            a += C39067sa3.a(20);
        }
        if ((this.a & SQLiteDatabase.OPEN_PRIVATECACHE) != 0) {
            a += C39067sa3.i(21, this.s0);
        }
        if ((this.a & ImageMetadata.LENS_APERTURE) != 0) {
            a += C39067sa3.s(22, this.t0);
        }
        if ((this.a & ImageMetadata.SHADING_MODE) != 0) {
            a += C39067sa3.s(23, this.u0);
        }
        if ((this.a & 2097152) != 0) {
            a += C39067sa3.s(24, this.v0);
        }
        if ((this.a & 4194304) != 0) {
            a += C39067sa3.s(25, this.w0);
        }
        if ((this.a & 8388608) != 0) {
            a += C39067sa3.s(26, this.x0);
        }
        if ((this.a & 16777216) != 0) {
            a += C39067sa3.s(27, this.y0);
        }
        if ((this.a & 33554432) != 0) {
            a += C39067sa3.s(28, this.z0);
        }
        if ((this.a & 67108864) != 0) {
            a += C39067sa3.a(29);
        }
        if ((this.a & 134217728) != 0) {
            a += C39067sa3.a(30);
        }
        if ((this.a & 268435456) != 0) {
            a += C39067sa3.s(31, this.C0);
        }
        if ((this.a & SQLiteDatabase.ENABLE_WRITE_AHEAD_LOGGING) != 0) {
            a += C39067sa3.s(32, this.D0);
        }
        if ((this.a & 1073741824) != 0) {
            a += C39067sa3.s(33, this.E0);
        }
        if ((this.a & Imgproc.CV_CANNY_L2_GRADIENT) != 0) {
            return C39067sa3.s(34, this.F0) + a;
        }
        return a;
    }

    @Override // com.google.protobuf.nano.MessageNano
    public final MessageNano mergeFrom(C36392qa3 c36392qa3) {
        while (true) {
            int u = c36392qa3.u();
            switch (u) {
                case 0:
                    break;
                case 8:
                    this.b = c36392qa3.f();
                    break;
                case 16:
                    this.c = c36392qa3.f();
                    break;
                case 24:
                    this.t = c36392qa3.q();
                    this.a |= 1;
                    break;
                case 32:
                    this.X = c36392qa3.q();
                    this.a |= 2;
                    break;
                case 40:
                    this.Y = c36392qa3.q();
                    this.a |= 4;
                    break;
                case 48:
                    this.Z = c36392qa3.q();
                    this.a |= 8;
                    break;
                case 56:
                    this.e0 = c36392qa3.q();
                    this.a |= 16;
                    break;
                case 64:
                    this.f0 = c36392qa3.q();
                    this.a |= 32;
                    break;
                case 72:
                    this.g0 = c36392qa3.q();
                    this.a |= 64;
                    break;
                case 80:
                    this.h0 = c36392qa3.q();
                    this.a |= 128;
                    break;
                case 88:
                    this.i0 = c36392qa3.q();
                    this.a |= 256;
                    break;
                case 96:
                    this.j0 = c36392qa3.q();
                    this.a |= Chrysalis.PIXEL_LAYOUT_ARGB;
                    break;
                case 104:
                    this.k0 = c36392qa3.q();
                    this.a |= 1024;
                    break;
                case 112:
                    this.l0 = c36392qa3.q();
                    this.a |= 2048;
                    break;
                case 120:
                    this.m0 = c36392qa3.q();
                    this.a |= 4096;
                    break;
                case 128:
                    this.n0 = c36392qa3.q();
                    this.a |= 8192;
                    break;
                case 136:
                    this.o0 = c36392qa3.q();
                    this.a |= 16384;
                    break;
                case Tweaks.CALLBACK_SYNC_ON_FEED_DELEGATE /* 144 */:
                    this.p0 = c36392qa3.q();
                    this.a |= SQLiteDatabase.OPEN_NOMUTEX;
                    break;
                case Tweaks.LOGIN_PREFETCH_PAGE_SIZE /* 152 */:
                    this.q0 = c36392qa3.q();
                    this.a |= 65536;
                    break;
                case Tweaks.ENABLE_PUBLIC_GROUPS /* 160 */:
                    this.r0 = c36392qa3.f();
                    this.a |= 131072;
                    break;
                case 168:
                    int q = c36392qa3.q();
                    if (q != 0 && q != 1 && q != 2 && q != 3 && q != 5 && q != 6 && q != 7) {
                        break;
                    } else {
                        this.s0 = q;
                        this.a |= SQLiteDatabase.OPEN_PRIVATECACHE;
                        break;
                    }
                case 176:
                    this.t0 = c36392qa3.q();
                    this.a |= ImageMetadata.LENS_APERTURE;
                    break;
                case 184:
                    this.u0 = c36392qa3.q();
                    this.a |= ImageMetadata.SHADING_MODE;
                    break;
                case 192:
                    this.v0 = c36392qa3.q();
                    this.a |= 2097152;
                    break;
                case AccountManagerConstants.AP_CONFIRM_CREDENTIAL_REQUEST_CODE /* 200 */:
                    this.w0 = c36392qa3.q();
                    this.a |= 4194304;
                    break;
                case 208:
                    this.x0 = c36392qa3.q();
                    this.a |= 8388608;
                    break;
                case 216:
                    this.y0 = c36392qa3.q();
                    this.a |= 16777216;
                    break;
                case 224:
                    this.z0 = c36392qa3.q();
                    this.a |= 33554432;
                    break;
                case 232:
                    this.A0 = c36392qa3.f();
                    this.a |= 67108864;
                    break;
                case 240:
                    this.B0 = c36392qa3.f();
                    this.a |= 134217728;
                    break;
                case 248:
                    this.C0 = c36392qa3.q();
                    this.a |= 268435456;
                    break;
                case 256:
                    this.D0 = c36392qa3.q();
                    this.a |= SQLiteDatabase.ENABLE_WRITE_AHEAD_LOGGING;
                    break;
                case 264:
                    this.E0 = c36392qa3.q();
                    this.a |= 1073741824;
                    break;
                case 272:
                    this.F0 = c36392qa3.q();
                    this.a |= Imgproc.CV_CANNY_L2_GRADIENT;
                    break;
                default:
                    if (!storeUnknownField(c36392qa3, u)) {
                        break;
                    } else {
                        break;
                    }
            }
        }
        return this;
    }

    @Override // defpackage.AbstractC32978o17, com.google.protobuf.nano.MessageNano
    public final void writeTo(C39067sa3 c39067sa3) {
        c39067sa3.z(1, this.b);
        c39067sa3.z(2, this.c);
        if ((this.a & 1) != 0) {
            c39067sa3.I(3, this.t);
        }
        if ((this.a & 2) != 0) {
            c39067sa3.I(4, this.X);
        }
        if ((this.a & 4) != 0) {
            c39067sa3.I(5, this.Y);
        }
        if ((this.a & 8) != 0) {
            c39067sa3.I(6, this.Z);
        }
        if ((this.a & 16) != 0) {
            c39067sa3.I(7, this.e0);
        }
        if ((this.a & 32) != 0) {
            c39067sa3.I(8, this.f0);
        }
        if ((this.a & 64) != 0) {
            c39067sa3.I(9, this.g0);
        }
        if ((this.a & 128) != 0) {
            c39067sa3.T(10, this.h0);
        }
        if ((this.a & 256) != 0) {
            c39067sa3.T(11, this.i0);
        }
        if ((this.a & Chrysalis.PIXEL_LAYOUT_ARGB) != 0) {
            c39067sa3.T(12, this.j0);
        }
        if ((this.a & 1024) != 0) {
            c39067sa3.T(13, this.k0);
        }
        if ((this.a & 2048) != 0) {
            c39067sa3.T(14, this.l0);
        }
        if ((this.a & 4096) != 0) {
            c39067sa3.T(15, this.m0);
        }
        if ((this.a & 8192) != 0) {
            c39067sa3.T(16, this.n0);
        }
        if ((this.a & 16384) != 0) {
            c39067sa3.T(17, this.o0);
        }
        if ((this.a & SQLiteDatabase.OPEN_NOMUTEX) != 0) {
            c39067sa3.T(18, this.p0);
        }
        if ((this.a & 65536) != 0) {
            c39067sa3.T(19, this.q0);
        }
        if ((this.a & 131072) != 0) {
            c39067sa3.z(20, this.r0);
        }
        if ((this.a & SQLiteDatabase.OPEN_PRIVATECACHE) != 0) {
            c39067sa3.I(21, this.s0);
        }
        if ((this.a & ImageMetadata.LENS_APERTURE) != 0) {
            c39067sa3.T(22, this.t0);
        }
        if ((this.a & ImageMetadata.SHADING_MODE) != 0) {
            c39067sa3.T(23, this.u0);
        }
        if ((this.a & 2097152) != 0) {
            c39067sa3.T(24, this.v0);
        }
        if ((this.a & 4194304) != 0) {
            c39067sa3.T(25, this.w0);
        }
        if ((this.a & 8388608) != 0) {
            c39067sa3.T(26, this.x0);
        }
        if ((this.a & 16777216) != 0) {
            c39067sa3.T(27, this.y0);
        }
        if ((this.a & 33554432) != 0) {
            c39067sa3.T(28, this.z0);
        }
        if ((this.a & 67108864) != 0) {
            c39067sa3.z(29, this.A0);
        }
        if ((this.a & 134217728) != 0) {
            c39067sa3.z(30, this.B0);
        }
        if ((this.a & 268435456) != 0) {
            c39067sa3.T(31, this.C0);
        }
        if ((this.a & SQLiteDatabase.ENABLE_WRITE_AHEAD_LOGGING) != 0) {
            c39067sa3.T(32, this.D0);
        }
        if ((this.a & 1073741824) != 0) {
            c39067sa3.T(33, this.E0);
        }
        if ((this.a & Imgproc.CV_CANNY_L2_GRADIENT) != 0) {
            c39067sa3.T(34, this.F0);
        }
        super.writeTo(c39067sa3);
    }
}
