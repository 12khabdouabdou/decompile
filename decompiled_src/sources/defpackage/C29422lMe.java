package defpackage;

import java.util.Set;

/* renamed from: lMe, reason: case insensitive filesystem */
/* loaded from: classes8.dex */
public class C29422lMe extends AbstractC24724hqj {
    public EnumC42797vMe j;
    public Z8d k;
    public Boolean l;
    public Boolean m;
    public String n;
    public EnumC38786sMe o;

    public C29422lMe() {
        super("REGISTRATION_USER_SIGNUP_USERNAME_PASSWORD_PAGEVIEW", EnumC1516Cre.BUSINESS, 1.0d, 1.0d, 1.0d);
    }

    @Override // defpackage.InterfaceC5193Jje
    public final void c(AK3 ak3, Set set) {
        byte[] bArr = new byte[1];
        AbstractC20835ew8.w0(ak3, 2, bArr, this.l, set);
        AbstractC20835ew8.B0(ak3, 4, bArr, this.n, set);
        AbstractC20835ew8.y0(ak3, 5, bArr, this.j, set);
        AbstractC20835ew8.w0(ak3, 6, bArr, this.m, set);
        AbstractC20835ew8.y0(ak3, 7, bArr, this.k, set);
        AbstractC20835ew8.y0(ak3, 8, bArr, this.o, set);
        ak3.m(bArr);
    }

    @Override // defpackage.InterfaceC5193Jje
    public final int d() {
        return 4738;
    }
}
