package defpackage;

import java.io.IOException;

/* renamed from: kxi, reason: case insensitive filesystem */
/* loaded from: classes9.dex */
public final class C28887kxi extends UVi {
    public final InterfaceC33754obi a;

    public C28887kxi(AG8 ag8) {
        this.a = AbstractC1490Cq9.c1(new YVi(ag8, new PWi(HLi.class)));
    }

    /* JADX WARN: Failed to find 'out' block for switch in B:10:0x002c. Please report as an issue. */
    @Override // defpackage.UVi
    /* renamed from: a, reason: merged with bridge method [inline-methods] */
    public C27550jxi read(DB9 db9) throws IOException {
        String A;
        String A2;
        if (db9.C() == 9) {
            db9.y();
            return null;
        }
        C27550jxi c27550jxi = new C27550jxi();
        db9.b = true;
        db9.b();
        while (db9.i()) {
            String w = db9.w();
            w.getClass();
            char c = 65535;
            switch (w.hashCode()) {
                case -1895722247:
                    if (w.equals("topsnap_volumes")) {
                        c = 0;
                        break;
                    }
                    break;
                case -1739819947:
                    if (w.equals("topsnap_media_type")) {
                        c = 1;
                        break;
                    }
                    break;
                case -1496981351:
                    if (w.equals("topsnap_audible_time_viewed_seconds")) {
                        c = 2;
                        break;
                    }
                    break;
                case -1043553210:
                    if (w.equals("rendered_timestamp_in_milli_seconds")) {
                        c = 3;
                        break;
                    }
                    break;
                case -781418641:
                    if (w.equals("topsnap_media_duration_seconds")) {
                        c = 4;
                        break;
                    }
                    break;
                case -358273109:
                    if (w.equals("creative_id")) {
                        c = 5;
                        break;
                    }
                    break;
                case -257485407:
                    if (w.equals("topsnap_max_continuous_time_viewed_seconds")) {
                        c = 6;
                        break;
                    }
                    break;
                case 504312333:
                    if (w.equals("delta_between_receive_and_render_millis")) {
                        c = 7;
                        break;
                    }
                    break;
                case 522677590:
                    if (w.equals("topsnap_time_viewed_seconds")) {
                        c = '\b';
                        break;
                    }
                    break;
                case 684215387:
                    if (w.equals("topsnap_time_viewed_before_interaction_seconds")) {
                        c = '\t';
                        break;
                    }
                    break;
                case 1542290869:
                    if (w.equals("topsnap_audio_playback_volume")) {
                        c = '\n';
                        break;
                    }
                    break;
            }
            switch (c) {
                case 0:
                    if (db9.C() == 9) {
                        db9.y();
                        break;
                    } else {
                        c27550jxi.h = (HLi) ((UVi) this.a.get()).read(db9);
                        break;
                    }
                case 1:
                    int C = db9.C();
                    if (C == 9) {
                        db9.y();
                        break;
                    } else {
                        if (C == 8) {
                            A = Boolean.toString(db9.n());
                        } else {
                            A = db9.A();
                        }
                        c27550jxi.k = A;
                        break;
                    }
                case 2:
                    if (db9.C() == 9) {
                        db9.y();
                        break;
                    } else {
                        c27550jxi.j = Float.valueOf((float) db9.o());
                        break;
                    }
                case 3:
                    if (db9.C() == 9) {
                        db9.y();
                        break;
                    } else {
                        c27550jxi.c = Long.valueOf(db9.q());
                        break;
                    }
                case 4:
                    if (db9.C() == 9) {
                        db9.y();
                        break;
                    } else {
                        c27550jxi.b = Float.valueOf((float) db9.o());
                        break;
                    }
                case 5:
                    int C2 = db9.C();
                    if (C2 == 9) {
                        db9.y();
                        break;
                    } else {
                        if (C2 == 8) {
                            A2 = Boolean.toString(db9.n());
                        } else {
                            A2 = db9.A();
                        }
                        c27550jxi.e = A2;
                        break;
                    }
                case 6:
                    if (db9.C() == 9) {
                        db9.y();
                        break;
                    } else {
                        c27550jxi.i = Float.valueOf((float) db9.o());
                        break;
                    }
                case 7:
                    if (db9.C() == 9) {
                        db9.y();
                        break;
                    } else {
                        c27550jxi.d = Long.valueOf(db9.q());
                        break;
                    }
                case '\b':
                    if (db9.C() == 9) {
                        db9.y();
                        break;
                    } else {
                        c27550jxi.a = Float.valueOf((float) db9.o());
                        break;
                    }
                case '\t':
                    if (db9.C() == 9) {
                        db9.y();
                        break;
                    } else {
                        c27550jxi.g = Float.valueOf((float) db9.o());
                        break;
                    }
                case '\n':
                    if (db9.C() == 9) {
                        db9.y();
                        break;
                    } else {
                        c27550jxi.f = Float.valueOf((float) db9.o());
                        break;
                    }
                default:
                    db9.K();
                    break;
            }
        }
        db9.g();
        return c27550jxi;
    }

    @Override // defpackage.UVi
    /* renamed from: b, reason: merged with bridge method [inline-methods] */
    public void write(C14496aC9 c14496aC9, C27550jxi c27550jxi) throws IOException {
        if (c27550jxi == null) {
            c14496aC9.j();
            return;
        }
        c14496aC9.Y = true;
        c14496aC9.c();
        if (c27550jxi.a != null) {
            c14496aC9.h("topsnap_time_viewed_seconds");
            c14496aC9.x(c27550jxi.a);
        }
        if (c27550jxi.b != null) {
            c14496aC9.h("topsnap_media_duration_seconds");
            c14496aC9.x(c27550jxi.b);
        }
        if (c27550jxi.c != null) {
            c14496aC9.h("rendered_timestamp_in_milli_seconds");
            c14496aC9.x(c27550jxi.c);
        }
        if (c27550jxi.d != null) {
            c14496aC9.h("delta_between_receive_and_render_millis");
            c14496aC9.x(c27550jxi.d);
        }
        if (c27550jxi.e != null) {
            c14496aC9.h("creative_id");
            c14496aC9.p(c27550jxi.e);
        }
        if (c27550jxi.f != null) {
            c14496aC9.h("topsnap_audio_playback_volume");
            c14496aC9.x(c27550jxi.f);
        }
        if (c27550jxi.g != null) {
            c14496aC9.h("topsnap_time_viewed_before_interaction_seconds");
            c14496aC9.x(c27550jxi.g);
        }
        if (c27550jxi.h != null) {
            c14496aC9.h("topsnap_volumes");
            ((UVi) this.a.get()).write(c14496aC9, c27550jxi.h);
        }
        if (c27550jxi.i != null) {
            c14496aC9.h("topsnap_max_continuous_time_viewed_seconds");
            c14496aC9.x(c27550jxi.i);
        }
        if (c27550jxi.j != null) {
            c14496aC9.h("topsnap_audible_time_viewed_seconds");
            c14496aC9.x(c27550jxi.j);
        }
        if (c27550jxi.k != null) {
            c14496aC9.h("topsnap_media_type");
            c14496aC9.p(c27550jxi.k);
        }
        c14496aC9.g();
    }
}
