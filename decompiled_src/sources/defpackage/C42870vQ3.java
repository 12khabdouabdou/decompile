package defpackage;

import java.util.Set;

/* renamed from: vQ3, reason: case insensitive filesystem */
/* loaded from: classes8.dex */
public class C42870vQ3 extends AbstractC48216zQ3 {
    public String D;
    public String E;
    public Long F;
    public Long G;
    public Long H;

    public C42870vQ3() {
        super("CONTENT_COMMENT_LONG_IMP", EnumC1516Cre.BEST_EFFORT, 0.1d, 0.1d, 1.0d);
    }

    @Override // defpackage.InterfaceC5193Jje
    public final void c(AK3 ak3, Set set) {
        byte[] bArr = new byte[4];
        AbstractC20835ew8.B0(ak3, 3, bArr, this.D, set);
        AbstractC20835ew8.y0(ak3, 4, bArr, this.B, set);
        AbstractC20835ew8.B0(ak3, 5, bArr, this.z, set);
        AbstractC20835ew8.y0(ak3, 6, bArr, this.v, set);
        AbstractC20835ew8.B0(ak3, 7, bArr, this.w, set);
        AbstractC20835ew8.y0(ak3, 8, bArr, this.A, set);
        AbstractC20835ew8.y0(ak3, 9, bArr, this.q, set);
        AbstractC20835ew8.B0(ak3, 10, bArr, this.p, set);
        AbstractC20835ew8.z0(ak3, 11, bArr, this.F, set);
        AbstractC20835ew8.z0(ak3, 12, bArr, this.G, set);
        AbstractC20835ew8.y0(ak3, 13, bArr, this.x, set);
        AbstractC20835ew8.z0(ak3, 14, bArr, this.C, set);
        AbstractC20835ew8.B0(ak3, 15, bArr, this.o, set);
        AbstractC20835ew8.y0(ak3, 16, bArr, this.j, set);
        AbstractC20835ew8.B0(ak3, 17, bArr, this.k, set);
        AbstractC20835ew8.B0(ak3, 18, bArr, this.E, set);
        AbstractC20835ew8.z0(ak3, 19, bArr, this.H, set);
        AbstractC20835ew8.y0(ak3, 20, bArr, this.l, set);
        AbstractC20835ew8.B0(ak3, 21, bArr, this.n, set);
        AbstractC20835ew8.z0(ak3, 22, bArr, this.m, set);
        AbstractC20835ew8.B0(ak3, 23, bArr, this.t, set);
        AbstractC20835ew8.z0(ak3, 24, bArr, this.s, set);
        AbstractC20835ew8.B0(ak3, 25, bArr, this.u, set);
        AbstractC20835ew8.y0(ak3, 26, bArr, this.y, set);
        AbstractC20835ew8.x0(ak3, 27, bArr, this.r, set);
        ak3.m(bArr);
    }

    @Override // defpackage.InterfaceC5193Jje
    public final int d() {
        return 5450;
    }
}
