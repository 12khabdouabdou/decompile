package defpackage;

import java.util.ArrayList;
import java.util.Set;

/* renamed from: Djg, reason: case insensitive filesystem */
/* loaded from: classes8.dex */
public class C1893Djg extends AbstractC24724hqj {
    public EU6 A;
    public C22428g8 B;
    public ArrayList C;
    public ArrayList D;
    public ArrayList E;
    public String j;
    public String k;
    public String l;
    public String m;
    public String n;
    public String o;
    public Long p;
    public Long q;
    public Long r;
    public Long s;
    public Long t;
    public EnumC37918rig u;
    public String v;
    public String w;
    public String x;
    public String y;
    public String z;

    public C1893Djg() {
        super("SHOPPING_SELECTION_STATE_EVENT", EnumC1516Cre.BUSINESS, 1.0d, 1.0d, 1.0d);
    }

    @Override // defpackage.InterfaceC5193Jje
    public final void c(AK3 ak3, Set set) {
        byte[] bArr = new byte[3];
        AbstractC20835ew8.B0(ak3, 2, bArr, this.k, set);
        AbstractC20835ew8.B0(ak3, 3, bArr, this.j, set);
        AbstractC20835ew8.B0(ak3, 5, bArr, this.o, set);
        AbstractC20835ew8.B0(ak3, 6, bArr, this.m, set);
        AbstractC20835ew8.B0(ak3, 7, bArr, this.n, set);
        AbstractC20835ew8.A0(ak3, 8, bArr, this.A, set);
        AbstractC20835ew8.K0(ak3, 9, bArr, this.C, set);
        AbstractC20835ew8.z0(ak3, 10, bArr, this.p, set);
        AbstractC20835ew8.z0(ak3, 11, bArr, this.s, set);
        AbstractC20835ew8.z0(ak3, 12, bArr, this.t, set);
        AbstractC20835ew8.y0(ak3, 13, bArr, this.u, set);
        AbstractC20835ew8.J0(ak3, 14, bArr, this.E, set);
        AbstractC20835ew8.B0(ak3, 15, bArr, this.l, set);
        AbstractC20835ew8.z0(ak3, 16, bArr, this.r, set);
        AbstractC20835ew8.z0(ak3, 17, bArr, this.q, set);
        AbstractC20835ew8.B0(ak3, 18, bArr, this.y, set);
        AbstractC20835ew8.K0(ak3, 19, bArr, this.D, set);
        AbstractC20835ew8.A0(ak3, 20, bArr, this.B, set);
        AbstractC20835ew8.B0(ak3, 21, bArr, this.z, set);
        AbstractC20835ew8.B0(ak3, 22, bArr, this.v, set);
        AbstractC20835ew8.B0(ak3, 23, bArr, this.x, set);
        AbstractC20835ew8.B0(ak3, 24, bArr, this.w, set);
        ak3.m(bArr);
    }

    @Override // defpackage.InterfaceC5193Jje
    public final int d() {
        return 2015;
    }
}
