package defpackage;

import java.util.Map;
import java.util.Set;

/* loaded from: classes8.dex */
public class X0b extends AbstractC24724hqj {
    public Long j;
    public Long k;
    public String l;
    public String m;
    public EnumC39075sab n;
    public Boolean o;
    public Double p;

    public X0b() {
        super("MAP_LENS_MARKER_TRAY_CLOSE", EnumC1516Cre.BUSINESS, 1.0d, 1.0d, 1.0d);
    }

    @Override // defpackage.InterfaceC5193Jje
    public final void c(AK3 ak3, Set set) {
        byte[] bArr = new byte[1];
        AbstractC20835ew8.y0(ak3, 2, bArr, this.n, set);
        AbstractC20835ew8.B0(ak3, 3, bArr, this.m, set);
        AbstractC20835ew8.z0(ak3, 4, bArr, this.j, set);
        AbstractC20835ew8.B0(ak3, 5, bArr, this.l, set);
        AbstractC20835ew8.x0(ak3, 6, bArr, this.p, set);
        AbstractC20835ew8.z0(ak3, 7, bArr, this.k, set);
        AbstractC20835ew8.w0(ak3, 8, bArr, this.o, set);
        ak3.m(bArr);
    }

    @Override // defpackage.InterfaceC5193Jje
    public final int d() {
        return 2846;
    }

    @Override // defpackage.AbstractC24724hqj, defpackage.MR6, defpackage.AXa
    public final int e(Map map) {
        int e = super.e(map);
        if (map.containsKey("close_method")) {
            Object obj = map.get("close_method");
            if (obj instanceof String) {
                this.n = EnumC39075sab.valueOf((String) obj);
            } else {
                this.n = (EnumC39075sab) obj;
            }
            e++;
        }
        String str = (String) map.get("lens_id");
        this.m = str;
        if (str != null) {
            e++;
        }
        Long l = (Long) map.get("map_session_id");
        this.j = l;
        if (l != null) {
            e++;
        }
        String str2 = (String) map.get("marker_id");
        this.l = str2;
        if (str2 != null) {
            e++;
        }
        Double d = (Double) map.get("open_time_sec");
        this.p = d;
        if (d != null) {
            e++;
        }
        Long l2 = (Long) map.get("tray_session_id");
        this.k = l2;
        if (l2 != null) {
            e++;
        }
        Boolean bool = (Boolean) map.get("was_lens_opened");
        this.o = bool;
        if (bool != null) {
            return e + 1;
        }
        return e;
    }
}
