package defpackage;

import java.util.Map;
import java.util.Set;

/* renamed from: Mmh, reason: case insensitive filesystem */
/* loaded from: classes8.dex */
public class C6886Mmh extends AbstractC24724hqj {
    public EnumC7430Nmh j;
    public String k;
    public String l;
    public Boolean m;
    public Long n;
    public String o;

    public C6886Mmh() {
        super("SPOTLIGHT_PLACE_TAGS", EnumC1516Cre.BUSINESS, 1.0d, 1.0d, 1.0d);
    }

    @Override // defpackage.InterfaceC5193Jje
    public final void c(AK3 ak3, Set set) {
        byte[] bArr = new byte[1];
        AbstractC20835ew8.y0(ak3, 2, bArr, this.j, set);
        AbstractC20835ew8.B0(ak3, 3, bArr, this.k, set);
        AbstractC20835ew8.B0(ak3, 4, bArr, this.l, set);
        AbstractC20835ew8.w0(ak3, 5, bArr, this.m, set);
        AbstractC20835ew8.z0(ak3, 6, bArr, this.n, set);
        AbstractC20835ew8.B0(ak3, 9, bArr, this.o, set);
        ak3.m(bArr);
    }

    @Override // defpackage.InterfaceC5193Jje
    public final int d() {
        return 4437;
    }

    @Override // defpackage.AbstractC24724hqj, defpackage.MR6, defpackage.AXa
    public final int e(Map map) {
        int e = super.e(map);
        if (map.containsKey("action")) {
            Object obj = map.get("action");
            if (obj instanceof String) {
                this.j = EnumC7430Nmh.valueOf((String) obj);
            } else {
                this.j = (EnumC7430Nmh) obj;
            }
            e++;
        }
        String str = (String) map.get("action_source");
        this.o = str;
        if (str != null) {
            e++;
        }
        Boolean bool = (Boolean) map.get("is_autoselected");
        this.m = bool;
        if (bool != null) {
            e++;
        }
        String str2 = (String) map.get("place_id");
        this.k = str2;
        if (str2 != null) {
            e++;
        }
        Long l = (Long) map.get("place_rank");
        this.n = l;
        if (l != null) {
            e++;
        }
        String str3 = (String) map.get("search_session_id");
        this.l = str3;
        if (str3 != null) {
            return e + 1;
        }
        return e;
    }
}
