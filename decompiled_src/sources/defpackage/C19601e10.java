package defpackage;

import com.snapchat.client.messaging.Tweaks;
import java.util.Set;

/* renamed from: e10, reason: case insensitive filesystem */
/* loaded from: classes8.dex */
public class C19601e10 extends AbstractC24724hqj {
    public String j;
    public Boolean k;
    public Long l;
    public Boolean m;
    public Boolean n;
    public Long o;
    public Long p;
    public String q;
    public String r;
    public String s;

    public C19601e10() {
        super("APP_STARTUP_FEED_READY", EnumC1516Cre.BUSINESS_CRITICAL, 1.0d, 1.0d, 1.0d);
    }

    @Override // defpackage.InterfaceC5193Jje
    public final void c(AK3 ak3, Set set) {
        byte[] bArr = new byte[2];
        AbstractC20835ew8.B0(ak3, 2, bArr, this.s, set);
        AbstractC20835ew8.B0(ak3, 3, bArr, this.r, set);
        AbstractC20835ew8.z0(ak3, 5, bArr, this.p, set);
        AbstractC20835ew8.B0(ak3, 8, bArr, this.j, set);
        AbstractC20835ew8.B0(ak3, 9, bArr, this.q, set);
        AbstractC20835ew8.z0(ak3, 10, bArr, this.o, set);
        AbstractC20835ew8.z0(ak3, 11, bArr, this.l, set);
        AbstractC20835ew8.w0(ak3, 13, bArr, this.n, set);
        AbstractC20835ew8.w0(ak3, 14, bArr, this.m, set);
        AbstractC20835ew8.w0(ak3, 15, bArr, this.k, set);
        ak3.m(bArr);
    }

    @Override // defpackage.InterfaceC5193Jje
    public final int d() {
        return Tweaks.ENABLE_LOGIN_PREFETCH;
    }
}
