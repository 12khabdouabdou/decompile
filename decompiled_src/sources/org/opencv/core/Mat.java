...
public class Mat {
    ...
    public int get(int i, int i2, byte[] bArr) {
        ...
        if (bArr != null && bArr.length % CvType.channels(type) == 0) {
            if (CvType.depth(type) != 0 && CvType.depth(type) != 1) {
                throw new UnsupportedOperationException(AbstractC31823n9f.m(type, "Mat data type is not compatible: "));
            }
    ...
    public Mat(Mat mat, Rect rect) {
        ...
        int i = rect.y;
        int i2 = i + rect.height;
        int i3 = rect.x;
        this.nativeObj = n_Mat(j, i, i2, i3, i3 + rect.width);
    }
