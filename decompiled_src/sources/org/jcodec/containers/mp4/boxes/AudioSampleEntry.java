...
public class AudioSampleEntry extends SampleEntry {
    ...
    public static AudioSampleEntry audioSampleEntryPCM(AudioFormat audioFormat) {
        ByteOrder byteOrder;
        String lookupFourcc = lookupFourcc(audioFormat);
        ...
        return audioSampleEntry(lookupFourcc, 1, sampleSizeInBits, channels, sampleRate, byteOrder);
    }
    ...
    public static Label[] getLabelsFromChan(ChannelBox channelBox) {
        ...
        while (i < values.length) {
            ...
            if (channelLayout2.getCode() == channelLayout) {
                if (channelLayout2 == ChannelLayout.kCAFChannelLayoutTag_UseChannelDescriptions) {
    ...
    public int calcFrameSize() {
        int i;
        if (this.version != 0 && (i = this.bytesPerFrame) != 0) {
            return i;
        }
    ...
    public ByteOrder getEndian() {
        ...
        if (endianBox == null) {
            ...
            if ("lpcm".equals(this.header.getFourcc())) {
                if ((this.lpcmFlags & kAudioFormatFlagIsBigEndian) != 0) {
                    return ByteOrder.BIG_ENDIAN;
                }
    ...
    public AudioFormat getFormat() {
        ...
        int calcSampleSize = calcSampleSize() << 3;
        short s = this.channelCount;
        if (getEndian() == ByteOrder.BIG_ENDIAN) {
            z = true;
        } else {
            z = false;
        }
        return new AudioFormat(i, calcSampleSize, s, true, z);
    }
